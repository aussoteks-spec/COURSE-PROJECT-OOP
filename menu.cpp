...
bool RoleMenu::bookAppointmentForPatient(const string& patientFIO) {
    system("cls");
    ...
    if (!filteredFreeSlots.empty()) {
        ...
        if (filteredIndex != -1) {
            string bookedSlot;
            if (doctors.bookAppointment(spec, docChoice, filteredIndex, bookedSlot)) {
        ...
        if (!allFreeSlots.empty()) {
            ...
            if (answer == "да" || answer == "д" || answer == "yes" || answer == "y") {
                ...
                if (slotChoice >= 1 && slotChoice <= allFreeSlots.size()) {
                    ...
                    if (realSlotIndex != -1) {
                        string bookedSlot;
                        if (doctors.bookAppointment(spec, docChoice, slotChoice - 1, bookedSlot)) {
                            DoctorInfo& doctor = docsList[docChoice];
                            string record = spec + " | " + doctor.fullName + " | " + bookedSlot;
...
bool RoleMenu::patientMenu(const string& login) {
    ...
    case 2: {
        bookAppointmentForPatient(patientFIO);
        system("pause");
        break;
    }
    ...
    case 3: {
        ...
        cout << "Запись отправлена в очередь отмены для подтверждения администратором.\n";
        system("pause");
        break;
    }
