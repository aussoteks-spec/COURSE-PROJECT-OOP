...
//сохраняет всех врачей и из расписание в файл 
...
//загружает данные врачей из файла
...
//создаёт расписание: 14 дней, 9:00-17:00
...
//преобразует строку времени в минуты для сортировки и сравнения
...
//добавляет нового врача с указанной специализацией
...
//находит врача по логину, возвращает специализацию и данные
...
//показывает все доступные специализации
bool DoctorSystem::showAllSpecs() {
    if (doctors.empty()) {
...
//показывает специализации с номерами для выбора
...
//показывает врачей конкретной специализации
...
//возвращает список врачей по специализации
...
//бронирует слот у врача
bool DoctorSystem::reserveAppointment(const string& spec, int doctorIndex, int slotIndex, string& outSlot) {
    try {
        if (!doctors.count(spec)) {
        ...
        for (int i = 0; i < doc.schedule.size(); i++) {
